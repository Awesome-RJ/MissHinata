...
def new_fed(update: Update, context: CallbackContext):
    ...
    if fednam != '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @MissLilly_Support if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @MissLilly_Support should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        fed_name = info['fname']
        #https://t.me/MissLilly_Support/41606 // https://t.me/MissLilly_Support/41619
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @MissLilly_Support."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @MissLilly_Support !"
            )
            return
